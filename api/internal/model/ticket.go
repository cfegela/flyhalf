...
func (r *TicketRepository) Create(ctx context.Context, ticket *Ticket) error {
	...
	return r.db.QueryRow(ctx, query,
		ticket.UserID, ticket.Title, ticket.Description, ticket.Status, ticket.Priority, ticket.AssignedTo,
...
func (r *TicketRepository) GetByID(ctx context.Context, id uuid.UUID) (*Ticket, error) {
	query := `
		SELECT id, user_id, title, description, status, priority, assigned_to, created_at, updated_at
		FROM tickets WHERE id = $1
	`
	ticket := &Ticket{}
	err := r.db.QueryRow(ctx, query, id).Scan(
		&ticket.ID, &ticket.UserID, &ticket.Title, &ticket.Description,
		&ticket.Status, &ticket.Priority, &ticket.AssignedTo, &ticket.CreatedAt, &ticket.UpdatedAt,
	)
	if err != nil {
		return nil, err
	}
...
func (r *TicketRepository) Update(ctx context.Context, ticket *Ticket) error {
	...
	`
	return r.db.QueryRow(ctx, query,
		ticket.Title, ticket.Description, ticket.Status, ticket.Priority, ticket.AssignedTo, ticket.ID,
