...
func (r *TicketRepository) Create(ctx context.Context, ticket *Ticket) error {
	query := `
		INSERT INTO tickets (user_id, title, description, status, assigned_to, epic_id, sprint_id, size, priority)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
		RETURNING id, created_at, updated_at
	`
	return r.db.QueryRow(ctx, query,
		ticket.UserID, ticket.Title, ticket.Description, ticket.Status, ticket.AssignedTo, ticket.EpicID, ticket.SprintID, ticket.Size, ticket.Priority,
...
func (r *TicketRepository) Update(ctx context.Context, ticket *Ticket) error {
	query := `
		UPDATE tickets
		SET title = $1, description = $2, status = $3, assigned_to = $4, epic_id = $5, sprint_id = $6, size = $7, priority = $8, updated_at = NOW()
		WHERE id = $9
		RETURNING updated_at
	`
	return r.db.QueryRow(ctx, query,
		ticket.Title, ticket.Description, ticket.Status, ticket.AssignedTo, ticket.EpicID, ticket.SprintID, ticket.Size, ticket.Priority, ticket.ID,
...
func (r *TicketRepository) GetNextLowerPriorityTicket(ctx context.Context, priority float64) (*Ticket, error) {
	query := `
		SELECT id, user_id, title, description, status, assigned_to, epic_id, sprint_id, size, priority, created_at, updated_at
		FROM tickets
		WHERE priority < $1
...
func (r *TicketRepository) GetNextHigherPriorityTicket(ctx context.Context, priority float64) (*Ticket, error) {
	query := `
		SELECT id, user_id, title, description, status, assigned_to, epic_id, sprint_id, size, priority, created_at, updated_at
		FROM tickets
		WHERE priority > $1
...
func (r *TicketRepository) GetSecondHigherPriorityTicket(ctx context.Context, priority float64) (*Ticket, error) {
	query := `
		SELECT id, user_id, title, description, status, assigned_to, epic_id, sprint_id, size, priority, created_at, updated_at
		FROM tickets
		WHERE priority > $1
...
func (r *TicketRepository) GetSecondLowerPriorityTicket(ctx context.Context, priority float64) (*Ticket, error) {
	query := `
		SELECT id, user_id, title, description, status, assigned_to, epic_id, sprint_id, size, priority, created_at, updated_at
		FROM tickets
		WHERE priority < $1
