...
func (h *TicketHandler) DemoteTicket(w http.ResponseWriter, r *http.Request) {
	_, ok := auth.GetUserID(r.Context())
	if !ok {
		http.Error(w, `{"error":"unauthorized"}`, http.StatusUnauthorized)
		return
	...
	// Find the ticket with the next lower priority
	lowerTicket, err := h.ticketRepo.GetNextLowerPriorityTicket(r.Context(), ticket.Priority)
	if err != nil {
		// No ticket below this one, nothing to do
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(ticket)
		return
	}
	...
	// Get updated ticket and return it
	updatedTicket, err := h.ticketRepo.GetByID(r.Context(), id)
	if err != nil {
		http.Error(w, `{"error":"ticket not found"}`, http.StatusNotFound)
		return
	}
